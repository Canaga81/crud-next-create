...
const Edit = () => {
    ...
    useEffect(() => {
        if (id) {
            axios.get(`https://65034c35a0f2c1f3faebc287.mockapi.io/blog/${id}`)
    ...
    const addHandler = async () => {
        try {
            await axios.put(`https://65034c35a0f2c1f3faebc287.mockapi.io/blog/${id}`, item);
            alert("Person updated!");
            router.push('/products');
        } catch (error) {
            alert("Person couldn't be updated: " + error);
        }
    }
    ...
    return (
        <>
            <div className='flex flex-col items-center justify-center h-screen gap-5 py-7 px-6'>
                <div>
                    <h1 className='font-bold text-[36px] tracking-[3px] text-center'>Edit Person</h1>
                </div>
                <div className='flex flex-col gap-4 items-center w-full max-w-[600px]'>
                    ...
                    <input 
                        value={item.blog} 
                        type='text' 
                        onChange={(e) => setItem({...item, blog: e.target.value})} 
                        className='w-full inline-block border-none outline-none text-xl font-normal bg-slate-200 text-black placeholder:text-black/70 py-3 px-10 rounded-md' 
                        placeholder="Haqqinda melumat yaziniz" 
                    />
                    <input 
                        value={item.img} 
                        type='text' 
                ...
                <div className='flex items-center gap-5'>
                    ...
                    <button onClick={addHandler} className="inline-block font-bold py-2 px-6 border-none outline-none bg-green-500 text-white rounded-xl text-[16px] transition-all hover:bg-opacity-85">Edit</button>
                </div>
            </div>
        </>
    );
